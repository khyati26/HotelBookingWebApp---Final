				...
				<div class="rowd-flex ">
					<div class=" col-12 ">
						<div class="card">
							...
							<%
								List<String> errorList = (List<String>) request.getAttribute("errMsgs");
								if (errorList != null) {
									for (String error : errorList) {
							%>
							<span style="color:red"><li><%=error%></li></span>
							<%
									}
								}
							%>
							<form class="form-card" method="post"
								  action="<%=request.getContextPath()+ AdminURLConstant.EDIT_CITYPAGE_URL %>"
												...
												<c:choose>
													<c:when test="${state.id == city.stateId }">
														<option value="${state.id }"
																selected>${state.stateName }</option>
													</c:when>
													<c:otherwise>
								...
								<div class="row justify-content-between text-left">
									<div class="form-group col-sm-6 flex-column d-flex">
										...
										<input type="text" id="cityname" value="${city.cityName }" name="cityName"
											   placeholder="Enter City Name" onblur="validate(3)" required>
									</div>
									...
									<table>
										<tr>
											<td>
												...
												<input type="file" id="cityimage" name="cityimage"
													   onblur="validate(7)"/>
											</td>
											<td>
												<img alt="no image" width="100px" height="100px"
													 src="<%=Constant.CITY_FOLDER_PARENTPATH%>${city.image}">
											</td>
										</tr>
									</table>
								</div>
