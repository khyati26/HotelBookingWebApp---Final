...
public class SearchHotelDAOMysql implements ISearchHotelDAO {

	public Set<Room> getAvailableRooms(SearchHotelCriteria searchHotel, int hotelid)
...
//				+ " select b.* from mydb.booking b where r.id=b.roomid ) and  r.hotelid = ? and r.maxcapacity <= ? ";
		...
		try {
			...
			if ((int) totalparsons % searchHotel.getRoom() == 0) {
				System.out.println(
						totalparsons / searchHotel.getRoom() == 2 || totalparsons / searchHotel.getRoom() == 1 ? 2
								: totalparsons / searchHotel.getRoom());
				ps.setInt(2, totalparsons / searchHotel.getRoom() == 2 || totalparsons / searchHotel.getRoom() == 1 ? 2
						: totalparsons / searchHotel.getRoom());
			} else {

				ps.setInt(2, (totalparsons / searchHotel.getRoom()) + 1 == 2 ? 3
						: (totalparsons / searchHotel.getRoom()) + 1);
			}
...
//						+ " b.checkin != ? and b.checkout != ? group by b.roomid ;\r\n";
			...
			throw new DatabaseException("Exception while getting available rooms in hotel: " , e);
	...
	public Set<Room> getAvailableRooms(SearchHotel searchHotel, int hotelid) throws DatabaseException, SQLException {
...
//						+ " b.checkin != ? and b.checkout != ? group by b.roomid ;\r\n";
	...
	public  List<Hotel> getAllHotelsInCity(int cityid) throws DatabaseException {
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		try {
			...
			throw new DatabaseException("Exception while getting city wise hotels 2: " , e);
		} finally {
			ConnectDB.closeResource(ps, rs);
		}
	}

	public  List<Hotel> getCityWiseHotels(SearchHotelCriteria searchHotelCriteria) throws DatabaseException {
		PreparedStatement ps = null;
		ResultSet rs = null;
		List<Hotel> hotelList = new ArrayList<>();
		...
		try {
			ps = ConnectDB.getConnection().prepareStatement(query1);
			ps.setString(1, searchHotelCriteria.getCity());
			int totalparsons = searchHotelCriteria.getAdult() + searchHotelCriteria.getChildren();
			
			if ((int) totalparsons % searchHotelCriteria.getRoom() == 0) {
				ps.setInt(2, totalparsons / searchHotelCriteria.getRoom() == 2 || totalparsons / searchHotelCriteria.getRoom() == 1 ? 3
						: totalparsons / searchHotelCriteria.getRoom());
			} else {
				ps.setInt(2, (totalparsons / searchHotelCriteria.getRoom()) + 1);
			}
			...
			throw new DatabaseException("Exception while getting city wise hotels: " , e);
		} finally {
			ConnectDB.closeResource(ps, rs);
		}
	}
